...
| 088 | تشديد الأمان — ثغرتان كشفهما المستشار | ✅ مُطبَّقة ٢٠٢٦-٠٩-٢٢ |
| 089 | سجلّ الاستيراد — من رفع وماذا رُفض ولماذا | ✅ مُطبَّقة ٢٠٢٦-٠٩-٢٣ |
| 090 | روابط «الإجراء التالي» تفتح تبويبها | ✅ مُطبَّقة ٢٠٢٦-٠٩-٢٣ |
| 091 | الوسوم — جدولان لا عمود نصّ | ✅ مُطبَّقة ٢٠٢٦-٠٩-٢٣ |
| 092 | تقنيع الاسم والهاتف عن التسويق | ✅ مُطبَّقة ٢٠٢٦-٠٩-٢٣ |
...
| سجلّ الاستيراد | `/dashboard/clients/import/history` | `crm_import_runs` (089) | ✅ تقرير الأخطاء يُنزَّل CSV |
...
| قراءة `clients` مباشرةً للتسويق | الاسم والهاتف مُقنَّعان في كل ما يعرضه النظام (092)، لكن نداءً مباشراً للواجهة البرمجية يُرجع الهاتف. إغلاقه يتطلّب تعريفاً ثانياً للنطاق — انظر §١٧ |
...
| اختبارات | ٢٥ (vitest) + ٣٤ (pgTAP) · تعمل في CI | — |
...
```sql
select * from tests.run_all();      -- ١٧: الحراسة والمرايا والرؤية والتقارير
select * from tests.run_roles();    -- ٩ : التسويق والمُطالِع تحت RLS حقيقية
select * from tests.run_privacy();  -- ٨ : التقنيع والوسوم
```

**وتعمل في CI** (`.github/workflows/ci.yml`): مهمّة للتطبيق (`tsc` ثم `vitest` ثم `build`) ومهمّة للقاعدة تُشغّل الثلاث عبر `psql`. ومهمّة القاعدة **تُعلن تخطّيها** إن غاب السرّ `SUPABASE_DB_URL` بدل أن تمرّ صامتة — فحصٌ يُتخطّى ويقول «نجح» أسوأ من فحص لا يوجد.
...
فُحصت القاعدة بمستشار Supabase بعد اكتمال الهجرات. ستّ ملاحظات، اثنتان عولجتا وأربع وُثّقت أسبابُ تركها.

### ما عولج

**١) `team_members`: عرضٌ للقراءة يُكتب من خلاله.** العرض `security definer` عمداً — شرط النطاق في `WHERE` داخله، فيتجاوز RLS ويُطبّق نطاقه بنفسه. لكنه عرضٌ بسيط على جدول واحد، أي **قابل للتحديث تلقائياً**، و`authenticated` يملك عليه `INSERT/UPDATE/DELETE`.

والنتيجة: مدير المتابعة يرى كل الموظفين في العرض فيستطيع تعديل صفوفهم أو حذفها من خلاله — بينما RLS على `employees` تقصر الكتابة على المدير والموارد البشرية. ثغرة كامنة لا مُستغَلّة (لا حساب بهذا الدور اليوم)، ولو أُنشئ غداً لعملت بلا أن يعلم أحد.

**٢) دوالّ ذات أثر جانبي مفتوحة لغير المسجَّل.** `EXECUTE` تُمنح لـ`PUBLIC` افتراضياً في Postgres، فورثتها الدوال القديمة. وكلها تفحص الصلاحية داخلها فلا ضرر اليوم — لكن الفحص الداخلي هو الخطّ الثاني لا الأول، ودالةٌ تُنهي خدمة موظف لا تُترك على مسافة نداءٍ من زائر.

### ما تُرك وسببه

| الملاحظة | لماذا تُركت |
|---|---|
| `pg_trgm` في `public` | نقله يكسر الفهرس `clients_name_trgm_idx` المبنيّ عليه، والمكسب أمنيّ نظريّ |
| `search_path` على دوالّ ليست `definer` | لا تعمل بصلاحية مرفوعة، فاختطاف `search_path` لا يمنح شيئاً |
| `security definer` على دوالّ الـCRM | مقصود: كلٌّ تفحص الصلاحية داخلها. والاستثناء المعاكس مقصود أيضاً — دوال التقارير (076) **بلا** definer كي تسري RLS |
| حماية كلمات السر المسرَّبة | إعداد في لوحة Supabase لا في SQL — **يُفعَّل يدوياً** من Authentication ← Policies |

## ١٥. الأداء والترقيم (§62)

كانت قائمة العملاء تجلب **كل** الصفوف بلا `range`، وسجلّ التواصل يقطع عند ٢٠٠ ويقول «(أحدث ٢٠٠)» بلا سبيل إلى ما قبلها. تعمل اليوم على ٦٧٤ عميلاً، وتتدهور صامتةً مع النمو — وهو أسوأ أنواع العطل: لا يظهر في اختبار ولا في شاشة، يظهر بعد سنة في شكوى بطء.

| الشاشة | قبل | بعد |
|---|---|---|
| قائمة العملاء | كل الصفوف | ٥٠ لكل صفحة + `count: exact` |
| سجلّ التواصل | ٢٠٠ بلا تنقّل | ٥٠ لكل صفحة + تنقّل |
| لوحة الفرص | ٥٠٠ صامتة | ٥٠٠ **مُعلَنة**: «معروضٌ ٥٠٠ من ١٢٠٠» |

والمُرقِّم يعرض **الإجمالي الحقيقي** لا عدد المعروض: قائمةٌ تقول «٥٠ عميلاً» وهي تعرض صفحةً من ستّمئة تكذب على من يبني عليها قراراً. والمُرشِّحات تُحمَل في الرابط مع التنقّل فلا يعود المتصفِّح إلى البداية.

ولوحة الأعمدة لا تُرقَّم عمداً — العمود يفقد معناه مقطوعاً. فتُقطع عند ٥٠٠ **ويُعلَن القطع**: شاشةٌ تعرض نصف الصورة بلا أن تقول ذلك أسوأ من شاشة تعتذر.

## ١٦. ملفّ العميل ٣٦٠ — التبويبات (§34)

صار الملفّ اثنتي عشرة لوحة، وقراءته تمريراً لا نظراً. والتبويب ليس تنظيماً بصرياً بل **إخفاءٌ مقصود**: الموظف يفتح الملفّ ليفعل شيئاً واحداً — يتصل، أو يسجّل، أو يبحث عن مستند — فيرى ما يخصّ ذلك.

| التبويب | فيه |
|---|---|
| نظرة | الإجراء التالي، الدرجة بأسبابها، الصحّة، الفرص، الوحدات المرشَّحة |
| التواصل | تسجيل التواصل والتسلسل الزمني |
| الصفقات | التأهيل و«فرصة جديدة» |
| العقار | الاهتمامات والحجز |
| المستندات | الرفع والتنزيل برابط موقَّع |
| البيانات | حقول العميل وجهة الاتصال البديلة والملاحظات |
| التاريخ | الملكية وتاريخ المراحل |

**وشريط الحالة يبقى فوق التبويبات دائماً**: المرحلة، وآخر تواصل، والمالك، والحرارة، وزرّا الاتصال والواتساب. إخفاؤه خلف تبويب يعني نقرةً قبل كل مكالمة.

والتبويب في `?tab=` لا في حالة المتصفّح وحدها، فروابط «الإجراء التالي» (090) تفتح تبويبها: «اعرض عليه وحدات» تفتح العقار، و«اسأل عن الميزانية» تفتح الصفقات. رابطٌ لا يصل أسوأ من رابط لا يوجد.

## ١٧. الخصوصية: ما بلغناه وما لم نبلغه (092)

**الاسم والهاتف مُقنَّعان للتسويق في كل ما يعرضه النظام:**

| القيمة | ما يراه المدير والموظف | ما يراه التسويق |
|---|---|---|
| الاسم | أحمد علي حسن | أحمد ع. |
| الهاتف | 07701234567 | 0770•••4567 |

والتقنيع في **مكان واحد**: `v_crm_opportunities` — وكل التقارير تقرأ منه (076)، فلا شاشة تنسى أن تُقنّع. ومعه `v_clients_masked` لمن احتاج قائمةً بلا هواتف. وشاشات تصفّح الأشخاص (العملاء، ملفّ العميل، الفرص) نُزعت من قائمة التسويق وحُرست في الصفحات نفسها — لا في الروابط وحدها، فمن يعرف المسار يكتبه.

### وما لم نبلغه — وأقوله صراحةً

نداءٌ مباشر للواجهة البرمجية (`GET /clients`) يُرجع للتسويق الهاتف **غير مُقنَّع**.

وسببه معماريّ لا إهمال: دوال التقارير كلها `security invoker` عمداً (076) كي تسري RLS فلا يرى الموظف أرقام الشركة. فالتسويق **يجب** أن يقرأ صفوف العملاء، وإلا عادت تقاريره **أصفاراً لا أخطاء** — وهو أسوأ: رقمٌ كاذب لا رسالة منع.

وإغلاقه يتطلّب أحد ثلاثة، كلٌّ أسوأ من الحدّ الحالي:

| المخرج | ثمنه |
|---|---|
| نزع قراءة الصفوف عن التسويق | تقاريره تصير أصفاراً صامتة |
| تحويل طبقة التقارير إلى `definer` بمُسنِد نطاق داخلها | تعريفان للنطاق يفترقان مع الوقت — وهي علّة 076 نفسها |
| عرضٌ `definer` للعملاء بمُرشِّح نطاقه | نفس الازدواج، في مكان ثالث |

فالضمانة هنا **تنظيمية لا تقنية**: التسويق موظّف بعقد، ولا يملك التصدير (الإدارة وحدها — §١٨)، وكل ما يفعله تحت سجلّ التدقيق. وكتابة هذا أصدق من ادّعاء إغلاقٍ لم يحدث.

## ١٨. الوسوم والإجراءات الجماعية (§47)

**الوسم جدولان لا عمود نصّ.** `tags text[]` أسهل بيوم وأغلى بسنة: يُكتب «VIP» و«vip» و«في آي بي» فتصير ثلاثة، ولا سبيل إلى إعادة تسمية وسم في كل الصفوف دفعةً. فالوسم صفٌّ له اسم ولون، والربط جدولٌ ثانٍ، والقائمة مغلقة يديرها المدير من إعدادات الـCRM.

وما ليس وسماً: المرحلة (`crm_stages`)، والحرارة (محسوبة في 074)، والمصدر (`crm_sources`). الوسم لما لا يُحسب ولا يُشتقّ.

**الإجراءات الجماعية** الآن خمسة: إسناد · مرحلة · وسم · مهمّة · تصدير. وكلها على المبدأ نفسه — الجماعي لا يعني بلا أثر فردي.

**والتصدير للإدارة وحدها.** البيانات الشخصية تخرج في ملفّ لا تحكمه RLS بعد خروجه، فمن يملك إخراجها هو من يملك حذفها. ويُصدَّر المعروض على الشاشة وحده — فما خرج هو ما رآه من أخرجه، ولا يُفاجأ بعمودٍ لم يعلم أنه في الملفّ.

## ١٩. المُرشِّحات الموحّدة (§44)

كانت كل لوحة تقرأ مُرشِّحاتها بطريقتها: التقارير `?days=30`، والقائمة `?filter=&temperature=`. فالانتقال بين اللوحات يُسقط ما اختاره القارئ — وأخطر منه أنه قد **لا ينتبه** أنه سقط، فيقارن رقم مشروع بعينه برقم الشركة كلها.

الآن مفردات واحدة: `from` · `to` · `owner` · `project` · `source` · `team`، بنفس الأسماء في الرابط وفي `CrmFilters` وفي معاملات دوال 076 — ثلاث طبقات بمفردات واحدة فلا يضيع مُرشِّح في الترجمة بينها.

والشريط `<form method="get">` بلا JavaScript: يعمل قبل أن يُحمَّل شيء. والمُرشِّح في الرابط يعني ثلاثة أشياء مجّاناً — يُشارَك كما هو، ويُحفَظ في «العروض المحفوظة» (§46)، ويعود بزرّ الرجوع.
