...
**الناقص: pgTAP.** السياسات والمحفّزات تُختبر اليوم يدوياً على القاعدة بنمط ثابت — كتلة `do $$ … raise exception ROLLBACK_OK … $$` تُنفّذ السيناريو ثم تُلغيه بالاستثناء وتحمل النتائج في رسالته. أثبت النمط قيمته: به وُجدت أخطاء §١٠ الستّة. لكنه يدويّ ولا يعمل في CI.
